...
class Solution(object):
    def findLadders(self, beginWord, endWord, wordList):
        ...
        # BFS on the graph_words
        cnt_level = 0
        cur_sq = {beginWord}
        oppo_nxt = {endWord}
        nxt_sq = set()
        a_wordList = set(wordList)
        b_wordList = set(wordList)
        b_wordList.discard(endWord)
        ...
        while not finished:
            ...
            for ew in cur_sq:
                if ew in oppo_nxt:
                    finished = True
                for iword in range(len(ew)):
                    for ic in range(26):
                        # compose a possible new word
                        ew_list = list(ew)
                        ew_list[iword] = chr(97 + ic)
                        new_word = ''.join(ew_list)
                        if a_side:
                            if new_word in a_wordList:
                                a_wordList.discard(new_word)
                                nxt_sq.add(new_word)
                                self.add_path_to_dict(ew, new_word)
                                self.add_path_to_dict(new_word, ew)
                        else:
                            if new_word in b_wordList:
                                b_wordList.discard(new_word)
                                nxt_sq.add(new_word)
                                self.add_path_to_dict(ew, new_word)
                                self.add_path_to_dict(new_word, ew)
            # after iterate through current sequence
            if len(nxt_sq) <= len(oppo_nxt):
                cur_sq = nxt_sq
            else:
                cur_sq = oppo_nxt
                oppo_nxt = nxt_sq
        ...
        self.total_step = cnt_level
        # print self.path_dict['ted']
        self.render_the_path(beginWord, [beginWord])
        return self.res

    def add_path_to_dict(self, st_pt, end_pt):
        if st_pt in self.path_dict:
            self.path_dict[st_pt].add(end_pt)
        else:
            self.path_dict[st_pt] = {end_pt}

    def render_the_path(self, cur_pt, cur_path):
        if cur_pt == self.endWord:
            new_path = list(cur_path)
            self.res.append(new_path)
        else:
            if len(cur_path) < self.total_step and cur_pt in self.path_dict:
                for neib in self.path_dict[cur_pt]:
                    if neib not in cur_path:
                        new_path = list(cur_path)
                        new_path.append(neib)
                        self.render_the_path(neib, new_path)
